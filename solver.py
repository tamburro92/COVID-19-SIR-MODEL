...
class Learner(object):
    ...
    def train(self):
        ...
        # calculate instant data
        recovered = recovered + deaths
        data = data - recovered

        optimal = minimize(self.loss, [0.2, 0.01], args=(data, recovered), method='L-BFGS-B', bounds=[(0.2, 0.4), (0.01, 0.1)])
        print(optimal)
...
def loss_weight(point, data, recovered):
    size = len(data)
    beta, gamma = point
    ...
    solution = solve_ivp(SIR, [0, size], [S_0, I_0, R_0], t_eval=np.arange(0, size, 1), vectorized=True)
    optim_days = 14  # Days to optimise for
    weights = 1/(np.arange(size+1)[1::][::-1])  # Recent data is more heavily weighted
    l1 = np.sqrt(np.mean((solution.y[1] - data) ** 2))
    l2 = np.sqrt(np.mean((solution.y[2] - recovered) ** 2))
    alpha = 0.5
    return alpha * l1 + (1 - alpha) * l2
